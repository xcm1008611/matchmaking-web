...
type BackLayoutMode = 'centered' | 'split'
...
const inscriptions = reactive({
  ...
  right: '永结同心',
  back: '永结同心',
  backMode: 'centered' as BackLayoutMode
})
...
// 沿爱心左/右侧内壁弧度从上向下流动排列姓名（符合正向阅读习惯：第一个字在上方，后续字向下延伸）
function drawCurvedHeartName(
  ...
  for (let i = 0; i < n; i++) {
    ...
    // 采样点分布：从上往下沿着爱心侧内弧延伸（i=0 在上方，后续字向下方延伸）
    const t = n <= 2 ? 0.22 + ratio * 0.56 : 0.12 + ratio * 0.76

    // 控制点：P0 (顶部内瓣) -> P1 (侧壁最凸处) -> P2 (底部内侧)
    ...
    const tiltAngle = (localAngle - Math.PI / 2) * 0.22 + heartRotation
...
// 侧边笔直向下的文字排版（分立模式：左右两侧各自笔直垂直向下竖排）
function drawStraightSideText(
  ctx: CanvasRenderingContext2D,
  text: string,
  side: 'left' | 'right',
  W: number,
  ...
  const xRatio = side === 'left' ? 0.18 : 0.82
  drawCenteredVerticalText(ctx, text, W * xRatio, H * 0.5, size, color, size * 0.25)
}

// 竖向单列文字（居中纵排，支持整体上下居中）
function drawCenteredVerticalText(
  ctx: CanvasRenderingContext2D,
  text: string,
  x: number,
  centerY: number,
  ...
  const totalHeight = n * size + (n - 1) * spacing
  const startY = centerY - totalHeight / 2 + size / 2
...
// 竖向多列书法排版（符合传统书法：从右至左换列，每列自上而下，整体中央居中）
function drawCalligraphyText(
  ...
  const rawText = text.trim() || '永结同心'
  // 若文字包含换行符，按换行拆分；否则按 4 字一列自动拆分
  let columns: string[] = []
  if (rawText.includes('\n')) {
    columns = rawText.split('\n').filter(s => s.length > 0)
  } else {
    const chars = Array.from(rawText)
    const colLen = chars.length <= 4 ? chars.length : Math.ceil(chars.length / 2)
    for (let i = 0; i < chars.length; i += colLen) {
      columns.push(chars.slice(i, i + colLen).join(''))
    }
  }

  const numCols = columns.length
  // 传统书法多列顺序：第一列在最右侧，后续列依次向左
  const totalW = (numCols - 1) * colSpacing
  const startX = centerX + totalW / 2 // 最右列 X 坐标

  columns.forEach((colStr, colIndex) => {
    const colX = startX - colIndex * colSpacing
    drawCenteredVerticalText(ctx, colStr, colX, centerY, size, color, spacing)
  })
}
...
// 正面整幅排版：精准复刻参考图（顶部居中「同心锁」大字 + 左右随弧远离中央镌刻铭文 + 中央交叉双心 + 姓名大字）
function drawFrontContent(ctx: CanvasRenderingContext2D, W: number, H: number) {
  ...
  // 1. 顶部居中标题「同心锁」（横向大字，从 0.088 放大至 0.115）
  drawHorizontalText(ctx, '同心锁', W * 0.5, H * 0.17, H * 0.115, COLOR_GOLD, H * 0.018)

  // 2. 左右两侧随心瓣弧度镌刻文字（从 0.062 调大至 0.070）
  const sideSize = H * 0.070
...
// 背面整幅排版：支持「中央书法居中」与「左右分立两侧」两种选项
function drawBackContent(ctx: CanvasRenderingContext2D, W: number, H: number) {
  const mode = inscriptions.backMode || 'centered'
  const size = H * 0.075
  ...
  if (mode === 'split') {
    ...
    const leftText = inscriptions.left || '心心相印'
    const rightText = inscriptions.right || '永结同心'
    drawStraightSideText(ctx, leftText, 'left', W, H, size, COLOR_GOLD)
    drawStraightSideText(ctx, rightText, 'right', W, H, size, COLOR_GOLD)
  } else {
    // 选项一（默认）：传统书法从右至左、自上而下竖排，锁面中央垂直居中
    const text = inscriptions.back || '永结同心'
    const fontScale = text.length > 6 ? H * 0.065 : H * 0.08
    drawCalligraphyText(ctx, text, W * 0.5, H * 0.5, fontScale, COLOR_GOLD, fontScale * 0.25, fontScale * 1.5)
  }
}
...
// 切换背面铭文排版模式（居中 vs 分立）
function setBackMode(mode: BackLayoutMode) {
  inscriptions.backMode = mode
  renderTextTextures()
}
            ...
            <div class="field-label-row">
              ...
              <div class="mode-switch-group">
                <button
                  ...
                  :class="{ 'is-active': inscriptions.backMode === 'centered' }"
                  @click="setBackMode('centered')"
                >
                  居中展示
                </button>
                <button
                  class="mode-switch-btn"
                  :class="{ 'is-active': inscriptions.backMode === 'split' }"
                  @click="setBackMode('split')"
                >
                  左右分立
                </button>
              </div>
        ...
        <div class="edit-panel">
          <div class="edit-fields">
            ...
            <div class="field">
              <div class="field-label-row">
                <label>背面 · 誓言铭文</label>
                <div class="mode-switch-group dark-mode-switch">
                  <button
                    class="mode-switch-btn"
                    :class="{ 'is-active': inscriptions.backMode === 'centered' }"
                    @click="setBackMode('centered')"
...
.mode-switch-btn.is-active {
  ...
  color: #ffffff;
  font-weight: 700;
  box-shadow: 0 2px 6px rgba(201, 75, 98, 0.25);
}
...
.dark-mode-switch .mode-switch-btn {
  color: rgba(240, 228, 218, 0.65);
}

.dark-mode-switch .mode-switch-btn:hover {
  color: #f7ede4;
}

.dark-mode-switch .mode-switch-btn.is-active {
  background: #c29353;
  color: #1f0d16;
  box-shadow: 0 2px 6px rgba(194, 147, 83, 0.3);
}
